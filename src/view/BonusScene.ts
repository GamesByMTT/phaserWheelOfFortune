...
export default class BonusScene extends Scene{
   ...
   SceneBg!: Phaser.GameObjects.Sprite
    ...
    create(){
        ...
        this.spinCircle = new Phaser.GameObjects.Sprite(this, 0, 0, 'spinCircle').setScale(0.6);
        ...
        for(let i=0; i< segments; i++){
            ...
            let text = this.add.text(0, 0, initData.gameData.BonusPayout[i], { font: "20px Arial", color: "#fff" });
            text.setOrigin(0.5);
            text.setPosition(
                360 * Math.cos(startAngle + (endAngle - startAngle) / 2),
                360 * Math.sin(startAngle + (endAngle - startAngle) / 2)
            );
